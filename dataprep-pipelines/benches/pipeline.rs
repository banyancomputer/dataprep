/* Deprecated for now, as this doesn't setup and use micro benchmarks */
...
use fake_file::{Strategy as FileStructureStrategy, Structure as FileStructure};
use fs_extra::dir;
...
// Defaults should be fine for benching on your dev machine
// Note, these defaults will probably raise warnings from criterion -- these are expected, since we are
// benchmarking a longer running process. We mess with the criterion configuration to get minimal
// iterations, which is not how you should use criterion, but it is how we use it here.
// If you need to change these, set the environment variables. We provide an example in the env
...
lazy_static! {
    ...
    static ref FILE_STRUCTURES_SIZE: usize = env::var("FILE_STRUCTURES_SIZE")
        .unwrap_or_else(|_| "1073741824".to_string()) // Default to 1 Gb
        .parse::<usize>()
        .unwrap();
    ...
    // Defaults to 1 seconds of warmup -- this basically forces criterion to use minimal iterations
    // Defaults to 1 seconds per sample -- this basically forces criterion to use minimal iterations
    // Defaults to 10 samples per file structure (the is the minimum)
