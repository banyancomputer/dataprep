...
//     // NOTE -- `cargo criterion` and `cargo bench` will treat non canonical paths differently
//     // Path we will use to hold our benchmarking data
//     static ref BENCH_PATH: String = env::var("BENCH_PATH").unwrap_or_else(|_| "test".to_string());
//     // Path we will use to hold inputs we build for benchmarking
//     static ref INPUT_PATH: String = env::var("INPUT_PATH").unwrap_or_else(|_| "test/input".to_string());
//     // Path we will use to hold packed data we build from inputs for benchmarking
//     static ref PACKED_PATH: String = env::var("PACKED_PATH").unwrap_or_else(|_| "test/packed".to_string());
//     // Path we will use to hold unpacked data we build from packed data for benchmarking
//     static ref UNPACKED_PATH: String = env::var("UNPACKED_PATH").unwrap_or_else(|_| "test/unpacked".to_string());
//     // Path we will use to hold the manifest files generated during benchmarking
//     static ref MANIFEST_PATH: String = env::var("MANIFEST_PATH").unwrap_or_else(|_| "test/manifest".to_string());
//     // Path to which write the benchmark results
//     static ref RESULT_PATH: String = env::var("RESULT_PATH").unwrap_or_else(|_| "test/result".to_string());
//
//     // Test Set Generation configuration
//     // Should we generate the test sets -- if set to false, you must provide the inputs at input_path
//     static ref GENERATE_FILE_STRUCTURES: bool = env::var("GENERATE_FILE_STRUCTURES")
...
//     static ref FILE_STRUCTURES_STRING: String = env::var("FILE_STRUCTURES").unwrap_or_else(|_| "simple".to_string());
//     // How big each test should be (in bytes) Try to use powers of 2 here
//     static ref FILE_STRUCTURES_SIZE: usize = env::var("FILE_STRUCTURES_SIZE")
//         // .unwrap_or_else(|_| "1048576".to_string()) // Default to 1Mb
//         .unwrap_or_else(|_| "4294967296".to_string()) // Default to 4 Gb
//         .parse::<usize>()
//         .unwrap();
//     // How wide a file structure can be
//     static ref FILE_STRUCTURES_MAX_WIDTH : usize = env::var("FILE_STRUCTURES_MAX_WIDTH")
//         .unwrap_or_else(|_| "4".to_string()) // Default to 4
//         .parse::<usize>()
//         .unwrap();
//     static ref FILE_STRUCTURES_MAX_DEPTH : usize = env::var("FILE_STRUCTURES_MAX_DEPTH")
//         .unwrap_or_else(|_| "4".to_string()) // Default to 4
//         .parse::<usize>()
//         .unwrap();
//
...
// }
//
// // /// Return the time it takes to run a function
// // /// # Arguments
// // /// * `f` - The function to run
// // /// # Returns
// // /// Result(T, Duration) - The result of the function, and the time it took to run
...
//     let bench_file_structures = FILE_STRUCTURES_STRING.as_str();
//     // Get the size of the test from the environment, as a usize
//     let bench_file_structure_size = *FILE_STRUCTURES_SIZE;
...
//     let max_width = *FILE_STRUCTURES_MAX_WIDTH;
//     // Get the max depth of the file structures
//     let max_depth = *FILE_STRUCTURES_MAX_DEPTH;
//
//     println!("Generating desired file structures:");
...
//         .unwrap();
//
//     // If GENERATE_FILE_STRUCTURES is true, then we need to generate the file structures
//     if *GENERATE_FILE_STRUCTURES {
//         // Populate the input directory with the desired file structures
//         populate_input_dirs();
//     }
//
...
//     // Prep the packed directory and manifest file
//     prep_pack(packed_path, manifest_path);
//
//     // Get the current time
//     let start = Instant::now();
//     // TODO (amiller68) - should I move here?
...
//
//     // Determine the throughput in bytes per second
//     let throughput_bps = (input_dir_size as f64) / (end.as_secs_f64());
//     // Determine the throughput in megabytes per second
//     let throughput_mbps = throughput_bps / 1000000.0;
//     // Determine the throughput in Gigabytes per second
//     let throughput_gbps = throughput_bps / 1000000000.0;
//
//     // Log the results to a file
//     let mut file = fs::File::create(result_path.join(format!("{}.txt", input_name))).unwrap();
//     file.write_all(
//         format!(
//             "\
//                 Packed {} in {} seconds\n\
...
//     // Prep the unpacked directory and manifest file
//     prep_unpack(unpacked_path, manifest_path);
//     // Get the current time
//     let start = Instant::now();
//     // The routine to benchmark
//     let _ = unpack_pipeline(
//         packed_path.clone(),
//         unpacked_path.clone(),
//         manifest_path.clone(),
//     )
...
//     #[tokio::test]
//     pub async fn pipeline_benchmark() -> Result<(), Error> {
...
//         let result_path = PathBuf::from(RESULT_PATH.as_str());
//
...
//             pack_benchmark(&input_path, &packed_path, &manifest_path, &result_path).await?;
//
//             // Run the unpack benchmark
//             unpack_benchmark(&packed_path, &unpacked_path, &manifest_path, &result_path).await?;
//
